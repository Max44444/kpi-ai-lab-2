...
def test(n):
    ...
    for i in range(n):
        ...
        statistics_2 = statistics(a_star, state_2)
    ...
    print(
        "{:>4} | {:>40} | {:>4} | {:>25} | {:>5} | {:>6} | {:100}".format("№", "start state", "alg", "time",
                                                                          "nodes",
                                                                          "moves", "path"))
    for i in range(n):
        v = results[i]
        ...
        print("{:>4} | {:>40} | {:>4} | {:>25} | {:>5} | {:>6} | {:100}"
              .format(i, ", ".join(map(str, v["start_state"][0])), "IDS", v["time_usage"][0],
                      v["node_usage"][0], v["moves"][0], ", ".join(map(str, v["path"][0]))))
        print("{:>4} | {:>40} | {:>4} | {:>25} | {:>5} | {:>6} | {:100}"
              .format("", "", "A*", v["time_usage"][1],
                      v["node_usage"][1], v["moves"][1], ", ".join(map(str, v["path"][1]))))
